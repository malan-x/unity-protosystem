...
namespace ProtoSystem.Sound
{
    ...
    public class UnitySoundProvider : ISoundProvider
    {
        ...
        public void Initialize(SoundManagerConfig config, SoundLibrary library)
        {
            ...
            Debug.Log($"[UnitySoundProvider] Initialized with pool size {config.audioSourcePoolSize}");
        }
        ...
        public SoundHandle Play(string id, Vector3? position = null, float volumeMultiplier = 1f)
        {
            ...
            if (entry == null)
            {
                Debug.LogWarning($"[UnitySoundProvider] Sound not found: {id}");
            ...
            if (source == null)
            {
                Debug.LogWarning($"[UnitySoundProvider] No available audio sources");
                return SoundHandle.Invalid;
            }
        ...
        public void PlayMusic(string id, float fadeInTime = 0f)
        {
            ...
            if (entry == null)
            {
                Debug.LogWarning($"[UnitySoundProvider] Music not found: {id}");
                return;
            }
        ...
        public void SetSnapshot(SoundSnapshotId snapshot, float transitionTime = 0.5f)
        {
            ...
            {
                Debug.LogWarning($"[UnitySoundProvider] Snapshot not found: {snapshot.Name}");
            }
        }
