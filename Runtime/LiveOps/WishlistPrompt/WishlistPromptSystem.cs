...
namespace ProtoSystem.LiveOps
{
    ...
    ///
    /// «Уже добавил» проверить нельзя: API чтения вишлиста у Steam нет. Кнопка
    /// нужна не для учёта, а чтобы не доставать человека, который уже согласился.
    /// </summary>
    ...
    public class WishlistPromptSystem : InitializableSystemBase
    {
        ...
        /// <summary>
        /// Оверлей Steam добавляет игру в желаемое, не выходя из игры. Он доступен
        /// не всегда (выключен игроком, не-Steam сборка) — тогда открываем
        /// страницу магазина в браузере.
        /// </summary>
        private void OpenStore()
        {
            ...
            if (config.steamAppId > 0 && SteamInitProvider.IsInitialized && SteamUtils.IsOverlayEnabled())
            {
                SteamFriends.ActivateGameOverlayToStore(
                    ...
                    EOverlayToStoreFlag.k_EOverlayToStoreFlag_AddToWishlist);
                return;
            }
#endif
