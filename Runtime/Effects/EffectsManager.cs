...
namespace ProtoSystem.Effects
{
    ...
    /// </summary>
    public class EffectsManager : InitializableSystemBase
    {
        ...
        public override System.Threading.Tasks.Task<bool> InitializeAsync()
        {
            ...
            {
                Debug.LogError("[EffectsManager] EffectContainer не назначен!");
                return System.Threading.Tasks.Task.FromResult(false);
            }

            if (!effectContainer.IsValid())
            {
                Debug.LogError($"[EffectsManager] EffectContainer '{effectContainer.ContainerName}' содержит ошибки!");
                return System.Threading.Tasks.Task.FromResult(false);
            }
            InitializePool();
            EnsureUICanvas();
        ...
        private void EnsureUICanvas()
        {
            ...
            Debug.Log("[EffectsManager] Создан Canvas для UI эффектов");
        }
        ...
        private void EnsureAudioSource()
        {
            ...
            Debug.Log("[EffectsManager] Создан AudioSource для эффектов");
#endif
        }
        ...
        private void InitializePool()
        {
            ...
            EffectInstance.SetEffectsManager(this);
        ...
        private void SetupAutoTriggers()
        {
            ...
            Debug.Log($"[EffectsManager] Настроено {_autoTriggerEffects.Count} триггеров запуска и {_autoStopEffects.Count} триггеров остановки");
        }
        ...
        public void PlayEffect(string effectId, Vector3 position = default, Transform parent = null)
        {
            ...
            if (config == null)
            {
                Debug.LogWarning($"[EffectsManager] Эффект '{effectId}' не найден!");
                return;
            }
        ...
        private void OnAutoTriggerEffect(int eventId, object data)
        {
            if (_autoTriggerEffects.TryGetValue(eventId, out var effects))
            {
                foreach (var effectConfig in effects)
                {
                    ...
                    {
                        Debug.LogWarning($"[EffectsManager] Эффект '{effectConfig.effectId}' требует IEffectTarget, но данные события не содержат позицию. Пропускаем.");
                        continue;
                    }

                    // Запускаем эффект с данными
                    PlayEffectWithSpawnData(effectConfig, hasSpawnData ? spawnData : default, hasSpawnData);
                    Debug.Log($"[EffectsManager] Автоматически запущен эффект '{effectConfig.effectId}' по событию {eventId}");
                }
            }
        }
        ...
        private void OnAutoStopEffect(int eventId, object data)
        {
            if (_autoStopEffects.TryGetValue(eventId, out var effects))
            {
                foreach (var effectConfig in effects)
                {
                    ...
                    Debug.Log($"[EffectsManager] Автоматически остановлен эффект '{effectConfig.effectId}' по событию {eventId}");
                }
            }
        }
    ...
    public class EffectInstance
    {
        ...
        public void Initialize(EffectConfig config, Vector3 position, Transform parent, Transform poolRoot = null)
        {
            ...
            // Получаем MonoBehaviour для корутин (используем EffectsManager если есть)
        ...
        /// <summary>
        /// Устанавливает корневой transform для кеша (вызывается из EffectsManager)
        /// </summary>
        ...
        /// <summary>
        /// Устанавливает ссылку на EffectsManager
        /// </summary>
        public static void SetEffectsManager(EffectsManager manager)
        {
