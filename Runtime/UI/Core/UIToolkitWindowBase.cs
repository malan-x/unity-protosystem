...
namespace ProtoSystem.UI
{
    ...
    public abstract class UIToolkitWindowBase : UIWindowBase
    {
        ...
        protected override void Awake()
        {
            ...
            // Запечённое окно, наоборот, ДОЛЖНО быть видно с первого кадра — ради этого его
            // и сохранили в сцене. Спрячем его здесь — и UISystem подхватит уже пустой экран,
            // а игрок до этого увидит голый 3D-мир (ровно то, от чего запекание и спасает).
            if (BakedInScene)
            {
                SkipShowAnimation = true;   // оно уже на экране: fade-in дал бы моргание
                return;
            }
