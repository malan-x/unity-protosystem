...
| Level | Поведение |
|-------|-----------|
| 0 | **Главные окна** — взаимоисключающие (открытие одного закрывает все другие) |
| 1+ | **Стековые окна** — накладываются поверх окон нижнего уровня, Back() возвращает к предыдущему |

**Правило закрытия:** при открытии окна уровня X автоматически закрываются все Normal окна уровня X и выше.

**Примеры:**
- MainMenu (Level 0) и GameHUD (Level 0) — не могут быть открыты одновременно (Level 0 закрывает другой Level 0)
- PauseMenu (Level 1) открывается поверх GameHUD (Level 0 сохраняется)
- Settings (Level 1) закрывает PauseMenu (Level 1) и открывается поверх GameHUD
- Inventory (Level 2) открывается поверх всех (Level 0, 1 сохраняются)
